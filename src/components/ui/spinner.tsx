...
const Circle = ({
	...
	size = 24,
	...props
}: SpinnerVariantProps & { children?: ReactNode }) => {
	if (children) {
		return (
...
const Ellipsis = ({ size = 24, ...props }: SpinnerVariantProps) => {
	const ellipsis1Id = useId();
	const ellipsis3Id = useId();
...
export const Spinner = ({ variant, children, ...props }: SpinnerProps) => {
	switch (variant) {
		case "circle":
